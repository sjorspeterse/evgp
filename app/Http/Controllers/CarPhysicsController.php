...
class CarPhysicsController extends Controller
{
    ...
    private function getUserListJSON() {
        ...
        $result = Cache::remember(
            "user_list", self::app_state_invalidation_time, function () {
            Log::debug("Could not retrieve list from cache, returning empty list");
                return "{}";
            }
        );
    ...
    public function getCarPhysicsList()
    {
        ...
        foreach($user_list as $user) {
            $physics_json = Cache::remember(
                $user->username, self::car_physics_invalidation_time, function () use ($user) {
                    // return $this->getCarPhysicsFromDB();
                    Log::debug("Could not find physics in cache for user " . $user->fullName . ", returning default");
